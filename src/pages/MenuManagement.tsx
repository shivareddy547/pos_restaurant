          ...
          <div key={item.id} onClick={(e) => handleCardClick(e, item)} className={`bg-white rounded-xl shadow-sm overflow-hidden transition-all cursor-pointer hover:shadow-md ${!item.available ? 'opacity-60' : ''} ${selectedItem?.id === item.id ? 'ring-2 ring-blue-500 ring-offset-2' : ''} ${flashItemId === item.id ? 'ring-2 ring-green-500 ring-offset-2 bg-green-50' : ''}`}>
            ...
            <div className="p-2 sm:p-4">
              <div className="flex items-start justify-between">
                ...
                <span className="text-sm sm:text-lg font-bold text-blue-600 ml-2 whitespace-nowrap">${Number(item.price).toFixed(2)}</span>
              </div>
    ...
    {selectedItem && (
      <div className="w-full lg:w-96 flex-shrink-0">
        <div className="bg-white rounded-xl shadow-sm p-6 lg:sticky lg:top-6 space-y-6">
          ...
          <div className="h-48 bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl flex items-center justify-center p-4">{selectedItem.image?.startsWith('data:image') ? (<img src={selectedItem.image} alt={selectedItem.name} className="w-full h-full object-contain rounded-lg" />) : (<span className="text-8xl">{selectedItem.image || '🍽️'}</span>)}</div>
          <div><h3 className="text-2xl font-bold text-gray-900">{selectedItem.name}</h3><p className="text-gray-500 capitalize mt-1">{selectedItem.category}</p><p className="text-3xl font-bold text-blue-600 mt-3">${Number(selectedItem.price).toFixed(2)}</p></div>
